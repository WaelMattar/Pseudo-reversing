...
plt.xlim([-3.4, 1.2])
plt.ylim([-1.2, 1.2])
plt.savefig('Figures/B_spline_roots.pdf', format='pdf')
...
for order in range(min_order, max_order):
    ...
    plt.plot(np.linspace(-len(gamma)/2+1, len(gamma)/2, len(gamma)), gamma, label='B-spline order = ' + str(order), alpha=1.5*(order-min_order)/max_order + 0.5, linewidth=3)
plt.tick_params(axis='both', which='major', labelsize=16)
plt.legend(prop={'size': 16}, loc='upper right')
plt.xlim([-15, 25])
plt.savefig('Figures/B_spline_decimation.pdf', format='pdf')
plt.show()
