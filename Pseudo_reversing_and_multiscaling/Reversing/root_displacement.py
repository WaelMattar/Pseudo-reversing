...
for xi in xis:
    ...
    plt.plot(np.linspace(-side_length - 1, side_length - 1, len(gamma)), gamma, label=r'$\xi = $' + str(np.round(xi, 4)), alpha=8/4*xi + 0.2, linewidth=3)
plt.xlim([-15, 25])
plt.tick_params(axis='both', which='major', labelsize=16)
plt.legend(prop={'size': 16}, loc='upper right')
plt.savefig('Figures/decimation_coefficients.pdf', format='pdf')
...
for xi in xis:
    ...
    plt.scatter([ele.real for ele in roots], [ele.imag for ele in roots], label=r'$\xi = $' + str(np.round(xi, 4)), marker='o', s=120 - 250*xi)
plt.legend(prop={'size': 16}, loc='upper right')
plt.tick_params(axis='both', which='major', labelsize=16)
plt.xlim([-1.8, 2.3])
plt.savefig('Figures/Reversing.eps', format='eps', bbox_inches='tight', pad_inches=0)
plt.show()
