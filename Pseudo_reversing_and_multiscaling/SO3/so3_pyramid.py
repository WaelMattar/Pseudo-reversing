...
for scale in range(1, pyramid_layers + 1):
    ...
    ax[scale - 1].set_ylim(0, np.max(norms[scale]) + 0.0, emit=True)
...
plt.savefig('Figures/SO3_pyramid.eps', format='eps')
...
plt.savefig('Figures/SO3_curve.eps', format='eps', bbox_inches='tight', pad_inches=0)
plt.show()
