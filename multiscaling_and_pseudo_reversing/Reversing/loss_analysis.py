...
def loss_function(alpha: list, support: list, xi: float):
    ...
    return np.linalg.norm(np.subtract(delta, np.convolve(gamma, even_alpha, 'valid')), ord=np.infty), np.linalg.norm(np.subtract(even_mask, even_alpha), ord=np.infty)
...
plt.ylim(bottom=0)
plt.xticks(lower_xi + np.linspace(lower_xi, upper_xi, 5))
plt.tick_params(axis='both', which='major', labelsize=16)
plt.xlabel(r'$\xi$', fontsize=15)
plt.legend(prop={'size': 16}, loc='upper left')
plt.savefig('Figures/loss_analysis.eps', format='eps')
plt.show()
